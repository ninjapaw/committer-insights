...
Beta.15 uses system-browser Microsoft sign-in. Beta.14 and earlier use the Windows account broker; cancel any pending attempt and restart with [beta.15](https://github.com/ninjapaw/committer-insights/releases/tag/v0.1.0-beta.15) to use the browser change. When testing an unpublished build, use `--skip-update-check` so it does not hand off to an older published release.

Beta.16 adds preparation stages and verified-file counts so a long runtime check no longer appears as one unchanged message. Cancellation and the existing sign-in deadline now interrupt cache verification. Full integrity checks remain mandatory. Preserve any reports, cancel pending sign-in, and restart with [beta.16](https://github.com/ninjapaw/committer-insights/releases/tag/v0.1.0-beta.16). This improves diagnostics and cancellation; the cause of the reported beta.15 stall and successful live Microsoft sign-in remain unconfirmed.

Beta.17 prepares the bundled Microsoft CLI before opening the app, without signing in. First launch extracts the runtime; later launches verify the existing cache. Sign In then uses the prepared runtime without extraction or another full file scan. Startup progress appears in the console. A preparation failure leaves GitHub available but requires an app restart to retry Microsoft preparation. Full integrity verification runs once per app launch; cache changes during that app session are not rechecked. See the [beta.17 release notes](docs/release-notes-beta.17.md).

The build now also removes unrelated Azure CLI command modules and service SDKs from a separate, verified copy before embedding it. Python, shared authentication dependencies, certificates and legal notices remain included. The single-executable download is preserved; the smaller payload is still unpacked locally before the app opens. Its own checksum and upstream source hash are recorded separately. Offline synthetic authentication checks run during the build. On 2026-09-25, the maintainer confirmed successful live Microsoft sign-in with the reduced candidate and cleared the beta.17 publication gate; this does not certify all tenant policies or live renewal. It is not a general-purpose CLI installation.

**Microsoft (Windows package):** **Sign in with Microsoft** and **Change account** use bundled Azure CLI 2.90.0 with Windows Web Account Manager (WAM) disabled only for the app's isolated CLI session. Microsoft account selection opens in your default browser; the application never requests your password. The CLI requests account selection rather than supplying a username. Device-code authentication is not forced; any device fallback when a browser cannot open is controlled by the CLI, not an automatic SDK retry. There is no separate Azure CLI button and no publisher application ID is required for this default path. First launch extracts and verifies the runtime; allow additional startup time and about 106 MB for this version's Azure CLI cache, plus temporary extraction space. Policies requiring broker-specific authentication may reject browser sign-in; the app does not bypass them.
...
New report drafts enable provider-reported billing for Azure DevOps and GitHub by default; either can be disabled in report options. Activity collection and the initial results view default to 90 days, capped to available report data. Current settings, provider billing snapshots and monthly aggregates retain their own dates rather than being relabeled as 90-day data.

Azure DevOps source review runs the same read-only dataset collectors as report generation. It shows each selected product estimate, security settings, visible repository history and requested billing dataset separately. A readable estimate is not proof of Git or billing access: partial sources remain usable but are labeled **Partial data access**. HTTP rejection, unsupported responses, timeouts and incomplete coverage are distinguished. Hidden or denied projects/repositories may be absent even after successful reads; ask the organization administrator to check project visibility, repository Read, Advanced Security access, billing permissions and explicit Deny when needed. No permissions are changed.

**Azure DevOps provider-reported billing:** per-product billing snapshots and identities are selected by default, with an optional UTC billing date and separately selected diagnostic details (off by default). Reports retain provider counts, subscription scope, collection status, and available project/repository/push evidence. Unmatched diagnostic identities are not added to totals. Reconciled counts cover only the selected organizations with complete same-date identity lists, not an entire subscription or invoice. Diagnostic data can include personal identities and email addresses.
...
The default Azure CLI flow uses a fresh temporary CLI directory for each sign-in, separate from your existing CLI configuration. CLI telemetry, dynamic extension installation and Windows broker use are disabled for that session. Browser cookies remain managed by your browser. App sign-out and cache removal do not sign you out of your browser or Windows, clear credentials retained by earlier broker-based versions, or revoke issued tokens. The app-owned directory is removed on cancellation, account change, sign-out, or normal process exit; a crash or forced termination may leave sensitive files under your temporary directory (`committer-azure-sign-in-*`). Remove those leftovers only when the application is closed. The verified tool runtime remains under `%LOCALAPPDATA%\CommitterInsights\tools\azure-cli`.

Reports can contain organization and repository identifiers, project metadata, contributor identities, activity, security settings, timestamps, and optional billing amounts. Source files, commit messages, patches, and GitHub author email addresses from activity collection are not retained. GitHub security billing, selected by default, includes provider-reported last-push email addresses and logins. Exports may contain personal and commercially sensitive information: apply your organization's retention and sharing policies.
