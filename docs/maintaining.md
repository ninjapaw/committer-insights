...
Azure enablement scenarios retain `uniqueCommitterCount` independently of returned names through the estimate adapter's optional structured callback. Legacy identity-only callers remain strict; incomplete names do not become a complete zero-count result. Report collection queries each selected product independently, preserving partial successes. Azure preflight uses the same collectors (without saving a report), returns dataset-level `accessChecks`, and labels partial evidence rather than equating one readable estimate with full access. It checks visible history for the requested window and requested billing details; this costs a collection pass, and report generation rechecks current access. Empty inventory is not verified history access, hidden projects cannot be certified, and uncollected invoice meters are not permission failures. The comparison remains available when billing collection is disabled or denied, but never interprets that as zero charges. New UI drafts enable reported billing, leave Azure diagnostic details off, and use 90-day activity/results defaults while retaining provider-specific billing dates. Standalone-product list prices are shared in contracts; scenario amounts are per organization/product and must not be merged with billed usage or across subscriptions. `azureEstimates` and its provenance are included only in the Azure provider section and all exports. Dates, incomplete coverage, mismatches and legacy-bundle uncertainty must remain visible.
