...
<title>${escapeHtml(PRODUCT.displayName)} - ${escapeHtml(report.subject)}</title>
...
</head><body><header><div class="header-inner"><div class="brand"><div class="logo" aria-hidden="true"><span></span><span></span><span></span><span></span></div><h1>${escapeHtml(PRODUCT.displayName)}</h1></div><div class="generated">Generated <time datetime="${escapeHtml(report.generatedAt)}" title="${escapeHtml(report.generatedAt)}">${escapeHtml(formatReportDateTime(report.generatedAt))}</time></div></div></header><main>`;
  const body =
    report.provider === 'combined'
