...
import { fetchAzureDevOpsEstimate, AzureDevOpsAdapterError } from '../../src/adapters/azure-devops/estimate-client.js';
...
const ESTIMATE_URL = 'https://advsec.dev.azure.com/contoso/_apis/management/meterUsageEstimate/default';
...
describe('fetchAzureDevOpsEstimate integration', () => {
    ...
    server.use(
      http.get(ESTIMATE_URL, () =>
        HttpResponse.json({
          ...
          billedUsers: [{ cuid: 'c1', userId: 'u1', descriptor: 'd1', displayName: 'Jane Doe', uniqueName: 'jane@contoso.com' }],
        }),
      ),
    );
    const result = await fetchAzureDevOpsEstimate({ organization: 'contoso', plan: 'codeSecurity', resultType: 'estimated', accessToken: 'token' });
  ...
  it('handles an empty billedUsers array', async () => {
    server.use(http.get(ESTIMATE_URL, () => HttpResponse.json({ uniqueCommitterCount: 0, billedUsers: [] })));
    const result = await fetchAzureDevOpsEstimate({ organization: 'contoso', plan: 'secretProtection', resultType: 'estimated', accessToken: 'token' });
  ...
  it('rejects a malformed upstream response', async () => {
    ...
    await expect(
      fetchAzureDevOpsEstimate({ organization: 'contoso', plan: 'all', resultType: 'estimated', accessToken: 'token' }),
    ).rejects.toBeInstanceOf(AzureDevOpsAdapterError);
  });

  it('maps 401 to authentication_required without retry', async () => {
    let calls = 0;
    server.use(http.get(ESTIMATE_URL, () => { calls += 1; return new HttpResponse(null, { status: 401 }); }));
    await expect(
      fetchAzureDevOpsEstimate({ organization: 'contoso', plan: 'codeSecurity', resultType: 'estimated', accessToken: 'token' }),
  ...
  it('respects Retry-After on 429 and eventually succeeds', async () => {
    ...
    const result = await fetchAzureDevOpsEstimate({ organization: 'contoso', plan: 'codeSecurity', resultType: 'estimated', accessToken: 'token', maxRetries: 2 });
    expect(result).toEqual([]);
  ...
  it('retries transient 5xx and eventually surfaces upstream_unavailable if exhausted', async () => {
    ...
    await expect(
      fetchAzureDevOpsEstimate({ organization: 'contoso', plan: 'codeSecurity', resultType: 'estimated', accessToken: 'token', maxRetries: 1 }),
